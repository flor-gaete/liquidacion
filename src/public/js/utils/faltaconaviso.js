...
let arrayDepdf = [];
arrayDepdf.push([
  "Agentes con faltas c/aviso igual o superiores a 4",
  "*******",
  "*******",
]);
arrayDepdf.push(["Nombre y Apellido", "Cuil", "Razones Particulares"]);
let TotalDeLegajos;
        ...
        arrayDeLegajos.forEach((legajo) => {
          ...
          fetch(urlIndividual, { method: "GET", headers: headers }).then(
            async (response) => {
              ...
              contador = contador + 1;
              if (
                jsonData.entry.properties["tc:dependencia"] === "HTC-Miembros"
              ) {
                contadorDeMiembros += 1;
                arrayDeMiembros.push(
                  jsonData.entry.properties["tc:apellido-y-nombre"].replace(
                    "-",
                    " "
                  )
                );
              ...
              if (jsonData.entry.properties["tc:razones-particulares"] >= 4) {
                ...
                arrayDeFaltadores.push({
                  name: nyaSinEspacios,
                  cuil: jsonData.entry.properties["tc:cuilok"],
                  razones: jsonData.entry.properties["tc:razones-particulares"],
                });
                /* respuesta.innerHTML += `
                                            <tr>
...
function show() {
  if (contador == 232) {
    ...
    console.info(arrayDeFaltadores);
    arrayDeFaltadores.forEach((persona) => {
      respuesta.innerHTML += `
                        <tr >
                        <td>${persona.name}</td>
                        <td>${persona.cuil}</td>
                        <td>${persona.razones}</td>
                        </tr>
                    `;
    });
...
function descargarpdf() {
  ...
  arrayDeFaltadores.forEach((objeto) => {
    //
...
/* const tabla = document.getElementById("tabla-faltaconaviso");
tabla.addEventListener("DOMContentLoaded", () => {
